...
template <typename T>
bool out_with_bitrev(const T* const input, const uint32_t n) {
  int i = 0;
  std::vector<T> a(2 * n);
...
template <typename T>
void stockham_fft_power2_time(T* a, T* b, const uint32_t n,
                              const T sign = 1.0) {
  ...
  // alpha = 2^L
  // beta = 2^(p - L - 1)
  for (uint32_t alpha = 1 /* L=0 */; alpha < n /* L = p - 1 */; alpha <<= 1) {
    {
      auto tmp = a;
      a        = b;
      b        = tmp;
    }
    const uint32_t beta = n / alpha / 2;
    for (uint32_t j = 0; j < beta; j++) {
      const T theta = sign * static_cast<T>(2.0 * M_PI) * beta / n;
      for (uint32_t k = 0; k < alpha; k++) {
        const T omega_r = std::cos(k * theta);
        const T omega_i = std::sin(k * theta);

        const uint32_t id1 = (j + beta) * alpha + k;
        const T tmp = std::fma(b[2 * id1], omega_r, -b[2 * id1 + 1] * omega_i);
        b[2 * id1 + 1] =
            std::fma(b[2 * id1], omega_i, b[2 * id1 + 1] * omega_r);
        b[2 * id1] = tmp;

        const uint32_t id0   = j * alpha + k;
        const uint32_t next0 = j * 2 * alpha + k;
        a[2 * next0]         = b[2 * id0] + b[2 * id1];
        a[2 * next0 + 1]     = b[2 * id0 + 1] + b[2 * id1 + 1];
        const uint32_t next1 = j * 2 * alpha + k + alpha;
        a[2 * next1]         = b[2 * id0] - b[2 * id1];
        a[2 * next1 + 1]     = b[2 * id0 + 1] - b[2 * id1 + 1];
      }
    }
  }
#else
  ...
  for (uint32_t alpha = 1 /* L=0 */; alpha < n /* L = p - 1 */; alpha <<= 1) {
    ...
    for (uint32_t k = 0; k < alpha; k++) {
      ...
      for (uint32_t j = 0; j < beta; j++) {
        const T omega_r    = std::cos(k * theta);
        const T omega_i    = std::sin(k * theta);
        const uint32_t id1 = k * 2 * beta + j + beta;
        const T tmp = std::fma(b[2 * id1], omega_r, -b[2 * id1 + 1] * omega_i);
        b[2 * id1 + 1] =
            std::fma(b[2 * id1], omega_i, b[2 * id1 + 1] * omega_r);
        b[2 * id1] = tmp;

        const uint32_t id0   = k * 2 * beta + j;
        const uint32_t next0 = k * beta + j;
        a[2 * next0]         = b[2 * id0] + b[2 * id1];
        a[2 * next0 + 1]     = b[2 * id0 + 1] + b[2 * id1 + 1];
        const uint32_t next1 = (k + alpha) * beta + j;
        a[2 * next1]         = b[2 * id0] - b[2 * id1];
        a[2 * next1 + 1]     = b[2 * id0 + 1] - b[2 * id1 + 1];
      }
    }
...
template <typename T>
void stockham_fft_power4_time(T* a, T* b, const uint32_t n,
                              const T sign = 1.0) {
  // a = {Re(a0), Im(a0), Re(a1), Im(a1), ..., Re(an-1), Im(an-1)}
...
std::vector<T> convolution(const std::vector<T>& a, const std::vector<T>& b) {
  ...
  stockham_fft_power2_time(ac.data(), workspace.data(), N);
  stockham_fft_power2_time(bc.data(), workspace.data(), N);
  for (size_t i = 0; i < N; i++) {
    const T i2    = 2 * i;
    const T tmp_r = ac[i2];
  ...
  stockham_fft_power2_time(ac.data(), workspace.data(), N, 1.0);
  std::vector<T> ret(n);
  for (size_t i = 0; i < n; i++) {
